...
class SavedRecord(DBAuthResource):
    ...
    def get(self, database, table, record):
        ...
        """
        data = Loader.get_database(database)[0 if table == 'MOLECULE' else 1][record]
    ...
    def delete(self, task):
        ...
        resp = dict(id=task, date=result.date, type=result.type, user=current_user)
        result.delete()
        return resp, 202
    ...
    @staticmethod
    def get_db():
        return {x: Loader.get_database(x)[self.__types[name][1]] for x in db_list}
