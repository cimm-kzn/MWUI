        ...
        if form == FormRoute.EDIT_PROFILE:
            ...
            def add_post():
                ...
                p = Meeting(meeting=active_form.meeting_id.data, deadline=active_form.deadline.data,
                            thesis_deadline=active_form.thesis_deadline.data, order=active_form.order.data,
                            thesis_count=active_form.thesis_count.data, thesis_types=active_form.thesis_types,
                            participation_types=active_form.participation_types, type=active_form.type,
                            author=current_user.get_user(), title=active_form.title.data, slug=active_form.slug.data,
            ...
            if active_form.validate_on_submit():
                if active_form.type != MeetingPostType.MEETING:
                    ...
                    if active_form.deadline.data and active_form.thesis_deadline.data:
                        return redirect(url_for('.blog_post', post=add_post()))
                    active_form.deadline.errors = ["Need deadline"]
                    active_form.thesis_deadline.errors = ["Need deadline"]
