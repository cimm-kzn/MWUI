    ...
    def dispatch_request(self, post):
        ...
        if admin:
            ...
            if remove_post_form.validate_on_submit():
                ...
                if hasattr(p, 'update_thesis_deadline') and edit_post.thesis_deadline.data:
                    p.update_thesis_deadline(edit_post.thesis_deadline.data)

                if hasattr(p, 'update_thesis_count') and edit_post.thesis_count.data:
                    p.update_thesis_count(edit_post.thesis_count.data)
        ...
        if p.classtype == 'Meeting':
            ...
            if p.type != MeetingPostType.MEETING:
                ...
                if current_user.is_authenticated and p.type == MeetingPostType.REGISTRATION \
                    ...
                    if special_form.validate_on_submit():
                        theses = list(Thesis.select(lambda x: x.post_parent == p.meeting and
                    ...
                    if sub and sub.type != MeetingPartType.LISTENER:
                        thesis_count = Thesis.select(lambda x: x.post_parent == p.meeting and
                                                     x.author == current_user.get_user()).count()
                        if thesis_count < p.meeting.thesis_count:
                            thesis_types = p.meeting.thesis_types
                            special_form = ThesisForm(prefix='special', body_name=p.body_name,
                                                      types=[x for x in
                                                             ThesisPostType.thesis_types(sub.type,
                                                                                         dante=thesis_count > 0)
                                                             if x in thesis_types])
                            if special_form.validate_on_submit():
            ...
            if current_user.is_authenticated and opened_by_author and p.meeting.thesis_deadline > datetime.utcnow():
                sub = Subscription.get(user=current_user.get_user(), meeting=p.meeting)
                thesis_types = p.meeting.thesis_types
                thesis_count = Thesis.select(lambda x: x.post_parent == p.meeting and
                                             x.author == current_user.get_user()).count()
